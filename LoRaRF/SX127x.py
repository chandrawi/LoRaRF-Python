...
class SX127x(BaseLoRA):
    ...
    STATUS_DEFAULT                         = LORA_STATUS_DEFAULT
    STATUS_TX_WAIT                         = LORA_STATUS_TX_WAIT
    STATUS_TX_TIMEOUT                      = LORA_STATUS_TX_TIMEOUT
    STATUS_TX_DONE                         = LORA_STATUS_TX_DONE
    STATUS_RX_WAIT                         = LORA_STATUS_RX_WAIT
    STATUS_RX_CONTINUOUS                   = LORA_STATUS_RX_CONTINUOUS
    STATUS_RX_TIMEOUT                      = LORA_STATUS_RX_TIMEOUT
    STATUS_RX_DONE                         = LORA_STATUS_RX_DONE
    STATUS_HEADER_ERR                      = LORA_STATUS_HEADER_ERR
    STATUS_CRC_ERR                         = LORA_STATUS_CRC_ERR
    STATUS_CAD_WAIT                        = LORA_STATUS_CAD_WAIT
    STATUS_CAD_DETECTED                    = LORA_STATUS_CAD_DETECTED
    STATUS_CAD_DONE                        = LORA_STATUS_CAD_DONE

    # Default Hardware Configuration
    PIN_NSS                                = 10
    PIN_RESET                              = 4
    SPI                                    = SPI
    SPI_FREQUENCY                          = F_CPU / 2
    ...
    _busyTimeout = 5000
    _spiSpeed = 7800000
    _txState = self.gpio.LOW
    _rxState = self.gpio.LOW
